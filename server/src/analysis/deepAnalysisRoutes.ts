...
router.post('/:stockCode', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/history', (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = getDeepReportHistory(stockCode, page, limit);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
router.get('/:reportId', (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const report = getDeepReport(reportId);
    if (!report) {
